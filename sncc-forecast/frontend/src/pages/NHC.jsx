...
export default function NHC() {
  ...
  const handleCellChange = (devKey, monthIso, field, value) => {
    ...
    setAssumptions(prev => {
      ...
      if (existing) {
        return prev.map(x => x === existing ? { ...x, [field]: Number(value) } : x)
      } else {
        return [...prev, {
  ...
  const FIELDS = [
    { key: 'sf_starts', label: 'SF Starts', type: 'number' },
    { key: 'mf_starts', label: 'MF Starts', type: 'number' },
    { key: 'sf_loan_amount', label: 'SF Loan Amt ($)', type: 'currency' },
    { key: 'mf_loan_amount', label: 'MF Loan Amt ($)', type: 'currency' },
    { key: 'sf_payoffs', label: 'SF Payoffs', type: 'number' },
    { key: 'mf_payoffs', label: 'MF Payoffs', type: 'number' },
  ]
                  ...
                  developments.map((devKey, devIdx) => {
                    ...
                    return FIELDS.map((field, fIdx) => (
                      ...
                      >
                        ...
                        {months.map((m, mIdx) => {
                          const val = getCell(devKey, m, field.key)
                          return (
                            <td key={mIdx} className="px-2 py-1">
                              <input
                                ...
                                defaultValue={val}
                                onBlur={e => handleCellChange(devKey, m, field.key, e.target.value)}
                                className="w-full text-right text-xs bg-transparent border border-transparent focus:border-blue-400 focus:bg-blue-50 rounded px-1 outline-none"
                              />
                            </td>
                          )
                        })}
