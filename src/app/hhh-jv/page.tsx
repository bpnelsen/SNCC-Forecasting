...
import {
  Handshake, Plus, Save, Trash2, X, AlertCircle, CheckCircle, Pencil,
} from 'lucide-react'
import { HHHJVProject, Builder, LoanProgram } from '@/lib/types'
import { formatCurrency } from '@/lib/utils'

type FormState = Omit<HHHJVProject, 'id'> & { id?: string }
...
export default function HHHJVPage() {
  const [projects, setProjects] = useState<HHHJVProject[]>([])
  ...
  const load = async () => {
    ...
    try {
      const [p, b, lp] = await Promise.all([
        fetch('/api/hhh-jv-projects').then(r => r.json()),
  ...
  const save = async () => {
    ...
    try {
      // POST for both create and update (some hosts reject PUT with a 405).
      const url = editing.id
        ? `/api/hhh-jv-projects/${editing.id}`
        : '/api/hhh-jv-projects'
  ...
  const remove = async (p: HHHJVProject) => {
    if (!confirm(`Delete "${p.name}"? This cannot be undone.`)) return
    setBusy(true); setMsg(null)
    try {
      const res = await fetch(`/api/hhh-jv-projects/${p.id}`, { method: 'DELETE' })
      ...
      <div className="flex items-center justify-between fade-up fade-up-1">
        <div>
          ...
          <p className="text-xs text-fg-dim mt-0.5">
            {projects.length} project(s) · joint-venture / HHH developments — not loans or land bucket projects
          </p>
        </div>
            ...
            <tbody>
              {projects.length === 0 ? (
                <tr><td colSpan={12} className="text-center text-xs text-fg-dim py-8">
                  No HHH/JV projects yet. Click <span className="text-accent">New Project</span> to add one.
                </td></tr>
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-surface border border-border-strong rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 grid grid-cols-2 gap-3">
          ...
          <Field label="Balance Outstanding ($)" hint="Feeds the HHH/JV forecast segment">
          ...
          <Field label="Vertical Loan Amount ($)" hint="Optional">
          ...
          <Field label="Dev Start" hint="Forecast starts contributing this month">
            <input type="date" className="form-input" value={form.dev_start_date ?? ''}
                   onChange={e => u({ dev_start_date: e.target.value || null })} />
          </Field>
          <Field label="Dev End" hint="Forecast contribution stops this month">
...
// Inline "+ Add builder" control. Lets the user add a builder without an SQL
// migration, then refreshes page state so it appears in the editor dropdown.
...
interface ProjectGroup {
  key: string
  builderName: string
  projects: HHHJVProject[]
}

// Group projects by builder, sort groups alphabetically (unassigned last),
// sort projects within each group by name.
function groupByBuilder(projects: HHHJVProject[], builders: Builder[]): ProjectGroup[] {
...
function BuilderGroup({
  ...
  programs: LoanProgram[]
  busy: boolean
  onEdit: (p: HHHJVProject) => void
  onDelete: (p: HHHJVProject) => void
}) {
      ...
      {group.projects.map(p => {
        ...
        return (
          <tr key={p.id}>
            ...
            <td className="text-[10px] font-mono">{p.dev_start_date ?? '—'}</td>
            <td className="text-[10px] font-mono">{p.dev_end_date ?? '—'}</td>
            <td>
...
function GrandTotalRow({ projects }: { projects: HHHJVProject[] }) {
...
function sumProjects(projects: HHHJVProject[]): {
  lots: number
  balance: number
