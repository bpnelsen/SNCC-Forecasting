-- Approved Loans pipeline — loans that have committee approval but have not
-- yet closed. The /approved page is a free-form tracker so an analyst can add
-- a row the moment a loan is approved and keep notes through closing.
--
...
-- Seed: current in-process pipeline (27 rows). Approval dates inferred from
-- the 90-day approval window — each lc_approval_expiration is exactly 90 days
-- past date_approved.
